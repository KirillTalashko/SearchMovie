...
object Migration_2_3_Impl : Migration(2, 3) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL(
            "DELETE FROM random_movie WHERE id NOT IN(SELECT MIN(id) FROM random_movie GROUP BY idMovieKp)"
        )
        db.execSQL(
        ...
        db.execSQL(
            "INSERT INTO random_movie_new" +
                    "(id, idMovieKp, name, url, ratingIMDb, ratingKp, duration, year, genres, type, description, date)" +
                    "SELECT id, idMovieKp, name, url, ratingIMDb, ratingKp, duration, year, genres, type, description, date FROM random_movie"
        )
